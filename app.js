/**
 * 旅人の杖と救いの泉 Ver 2.0.18
 * メインロジック（東海自然歩道・強制ネーミング ＆ 飲食店・屋号対応版）
 */
...
function renderGeoJson(key, bounds = null) {
    ...
    L.geoJSON(rawData[key], {
        ...
        onEachFeature: function(feature, layer) {
            ...
            // 🚨 forceName（強制上書き）を最優先にする！
            if (def.forceName) {
                name = def.forceName;
            } else {
                // 屋号、名前、地区名など、あらゆる可能性を探る
                name = p.屋号 || p.name || p.名称 || p.地区名 || p.観光資源名 || p.指定名称 || p.文化財名 || p.通称 || "名称未定";
                
                // 「0」や空文字が拾われてしまった場合の保険
                if (name === "0" || name === "" || name === null) name = "名称未定";
...
setTimeout(hideLoadingScreen, 4000); // ロードが詰まっても4秒後に強制開始！
